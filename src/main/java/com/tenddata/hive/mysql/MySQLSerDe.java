...
public class MySQLSerDe implements SerDe {
	...
	public void initialize(final Configuration conf, final Properties tbl)
		...
		for (int i = 0; i < columnNamesArray.length; i++) {
			...
			} else if (MySQLSerDe.HIVE_TYPE_SMALLINT.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
						.add(PrimitiveObjectInspectorFactory.javaShortObjectInspector);
			} else if (MySQLSerDe.HIVE_TYPE_TINYINT.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
						.add(PrimitiveObjectInspectorFactory.javaByteObjectInspector);
			} else if (MySQLSerDe.HIVE_TYPE_BIGINT.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
						.add(PrimitiveObjectInspectorFactory.javaLongObjectInspector);
			} else if (MySQLSerDe.HIVE_TYPE_BOOLEAN.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
						.add(PrimitiveObjectInspectorFactory.javaBooleanObjectInspector);
			} else if (MySQLSerDe.HIVE_TYPE_FLOAT.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
						.add(PrimitiveObjectInspectorFactory.javaFloatObjectInspector);
			} else if (MySQLSerDe.HIVE_TYPE_DOUBLE.equalsIgnoreCase(columnTypesArray[i])) {
				fieldOIs
